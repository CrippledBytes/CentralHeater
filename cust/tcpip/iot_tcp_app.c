...
void proces_data(char data[],int dataleng){
char token[100] = "";
memset(token, '\0', sizeof(token));
int portnr = 0;
    if(strlen(data) > 3){
        strncpy(token,data,3);
        if (strcmp(token,"set") == 0){
            if(strlen(data) > 10){
                memset(token, '\0', sizeof(token));
                strncpy(token,data + 4,4);
                if (strcmp(token,"GPIO") == 0){
                    memset(token, '\0', sizeof(token));
                    strncpy(token,data + 8,1);
                    portnr = atoi(token);
                    memset(token, '\0', sizeof(token));
                    strncpy(token,data + 10,1);
                    if (strcmp(token,"0") == 0){
						iot_gpio_output(portnr,0 ); 
                        printf_high("GPIO%d is set to off\n",portnr);
                    }
                    else if(strcmp(token,"1") == 0) {
						iot_gpio_output(portnr,1 );
                        printf_high("GPIO%d is set to on\n", portnr);
                    }
                    else {
                        printf_high("Wrong status :/\n");
                    }
                }
            ...
            printf_high("First is not a set");
        }
	}
	else {
        printf_high("data is not big enough!");
	}
	memset(token, '\0', sizeof(token));
}
#endif
