...
/*========================================================================
    ...
    Routine    Description:
        ...
        iot_atcmd_exec_send_temp -- command to send measurements to honeycomb
	...
	example1: AT#Temp -a254 -b554 -c58 -d587 -> Ingoing temp is 25.4°C, Outgoing temp is 55.4°C, Ambient temp is 5.8°C, Ambient humidity is 58.7%

========================================================================*/

int16 iot_atcmd_exec_send_temp(puchar pCmdBuf, int16 AtCmdLen)
{
    char *argv[MAX_OPTION_COUNT];
    char *opString = "a:b:c:d:?";
    char *endptr = NULL;
	char *result = "";
	char buffer[10] = "";
	int8 i = 0;
    ...
    uint16 eOffset = 0;
    uint8 eData = 0;
	int16 val[AANTAL_KANALEN] ={2000,2000,2000,2000};
	char *channels[AANTAL_KANALEN] = {"tin=","&tout=","&at=","&ah="};
    split_string_cmd(pCmdBuf, AtCmdLen, &argc, argv);
    opt = getopt(argc, argv, opString);
	char serverIP[16] = "";
	memcpy(serverIP, "192.168.0.142", 13);
	serverIP[13]='\0';
	uint32 serverIPlen = 0;
    ...
    while (opt != -1) {
        switch (opt) {
            ...
            case 'b':
                ...
                break;
            case 'c':
                /*optie c (omgevingstemp)*/
                num = simple_strtol(optarg,&endptr,0);
				val[2] = num;
                break;
			case 'd':
                /*optie d (vochtigheid omgeving) */
                num = simple_strtol(optarg,&endptr,0);
				val[3] = num;
                break;
			case 'e':
                /*optie e (ip adres server) */
                if (optarg == NULL)
                    break;
                
                serverIPlen = strlen(optarg);
                if (serverIPlen > 15) {
                    return -1;
                }
                memcpy(serverIP, optarg, serverIPlen);
	...
	strcpy(result,"/send.php?");
	len = strlen(result);
	while(i <= AANTAL_KANALEN-1){
		if ( val[i] != 2000) {
		strcpy(result + len, channels[i]);
		len = strlen(result);
	...
	i=0;
	printf_high("Current server IP: %s\n",serverIP);
	iot_uart_output((puchar)result, (int16)strlen(result));
	printf_high("\r\n");
	webclient_get(serverIP,80,result);

      //uip_send(result,strlen(result));
...
int16 iot_atcmd_parser(puchar cmd_buf, int16 AtCmdLen)
{
	...
	/*Format:		AT#Temp+enter*/
	    else if (!memcmp(cmd_buf,AT_CMD_SEND_TEMP,sizeof(AT_CMD_SEND_TEMP)-1)) {
        ret_code = iot_atcmd_exec_send_temp(cmd_buf, AtCmdLen);
    }
#if (ATCMD_SET_SMNT_SUPPORT == 1)
