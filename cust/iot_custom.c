...
void iot_cust_subtask1(void)
{
	...
	uint8 PinInput, Polarity;
	
	/*Make GPOI 2 blinking every 1 second*/
	subCurTimer = iot_get_ms_time();	/*get current counter value (unit:1ms)*/
	
	/*if the time difference is 1000ms, change gpoi output value*/
		if (((subCurTimer - subPreTimer) >=1000) || (subCurTimer < subPreTimer)){
			subPreTimer = subCurTimer;
			iot_gpio_read(2, &PinInput, &Polarity);	/*get gpio mode and value*/
			iot_gpio_output(2, (PinInput==0)?1:0);		/*change gpio to output mode with the reversed value*/
			}
